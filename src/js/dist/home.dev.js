...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (navOverlay) navOverlay.addEventListener('click', closeMenu); // Fecha ao clicar em qualquer link do menu
  ...
  if (track) {
    ...
    // Auto-play
    var startAuto = function startAuto() {
      autoTimer = setInterval(next, 4000);
    };

    var resetAuto = function resetAuto() {
    ...
    var autoTimer = null;
    var startX = 0;
    var isDragging = false; // Cria dots
  ...
  if (navbar) {
    ...
    window.addEventListener('scroll', function () {
      ...
      if (shouldScroll !== scrolled) {
        ...
        navbar.style.boxShadow = scrolled ? '0 4px 20px rgba(0,0,0,.08)' : 'none';
      }
    }, {
      passive: true
    });
